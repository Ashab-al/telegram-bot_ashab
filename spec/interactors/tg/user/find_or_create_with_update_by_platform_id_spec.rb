...
RSpec.describe Tg::User::FindOrCreateWithUpdateByPlatformIdInteractor do
  describe '#execute' do
    ...
    let(:chat_hash_user_second) do
      { id: user_second[:id], first_name: user_second[:name], username: user_second[:username] }
    end

    let(:user_status_blocked) { create(:user, bot_status: status_bot_blocked) }
    let(:chat_hash_user_blocked) do
      { id: user_status_blocked.platform_id, first_name: user_status_blocked.name,
        username: user_status_blocked.username }
    end
