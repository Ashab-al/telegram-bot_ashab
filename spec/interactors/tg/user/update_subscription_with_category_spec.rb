...
RSpec.describe Tg::User::UpdateSubscriptionWithCategoryInteractor do
  describe '#execute' do
    ...
    describe 'create subscription' do
      let(:subscribe_to_category) do
        described_class.run(user: user, category_name: category_second.name, subscribed_categories: user.category)
      end
    ...
    describe 'unsubscribe from category' do
      let(:unsubscribe_from_category) do
        described_class.run(user: user, category_name: category_first.name, subscribed_categories: user.category)
      end
