...
class TelegramWebhooksController < Telegram::Bot::UpdatesController
  ...
  IGNORED_FOR_USER_AND_SUBSCRIBED_CATEGORIES = %i[choice_category message session_key]
    ...
    when t('buttons.menu.points')
      ...
      respond_with :message, text: erb_render('menu/default', binding), parse_mode: 'HTML', reply_markup: {
        keyboard: [[erb_render('menu/button/points', binding), erb_render('menu/button/advertisement', binding), erb_render('menu/button/help', binding)],
                   [erb_render('menu/button/categories',
                               binding)]], resize_keyboard: true, one_time_keyboard: true, selective: true
      }
    end
  end
  ...
  def points
    points_message = respond_with :message,
                                  ...
                                  reply_markup: {
                                    inline_keyboard: [10, 30, 50, 100, 150,
                                                      200].map do |quantity|
                                      [{ text: t("buttons.by_points.point_#{quantity}"),
  ...
  def choice_category
    category_send_message = respond_with :message, text: "#{t('choice_category')}",
  ...
  def callback_query(data_callback)
    ...
    when /^mid_\d+_bdid_\d+/
      ...
      if @open_vacancy[:status] == :warning
        answer_callback_query erb_render(@open_vacancy[:path_view], binding),
                              show_alert: true
      end
                              ...
                              reply_markup: {
                                inline_keyboard: [
                                  ...
                                  [{ text: "#{I18n.t('buttons.for_vacancy_message.spam')}",
                                     callback_data: I18n.t('buttons.for_vacancy_message.callback_data',
  ...
  def update_point_send_messag(text, data, message_id, button)
    ...
    @user.update(data)
    bot.send_message(chat_id: Rails.application.secrets.errors_chat_id,
                     text: "update_point_send_messag Вакансия успешно отправлена ✅ err: #{e}")
  end
  ...
  def send_vacancies(vacancies, start_number_vacancy)
    ...
    vacancies.each do |vacancy|
      ...
      bot.edit_message_text(text: erb_render('pagination/vacancy', binding), message_id: message_id, chat_id: @user.platform_id, parse_mode: 'HTML',
                            reply_markup: {
                              inline_keyboard: [
                                [{ text: erb_render('button/get_contact', binding),
                                   callback_data: "mid_#{message_id}_bdid_#{@vacancy.id}" }],
                                [{ text: "#{I18n.t('buttons.for_vacancy_message.by_points')}",
                                   callback_data: "#{I18n.t('buttons.points')}" }],
                                [{ text: "#{I18n.t('buttons.for_vacancy_message.spam')}",
                                   callback_data: I18n.t('buttons.for_vacancy_message.callback_data',
